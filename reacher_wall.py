...
class ReacherWallEnv:
	...
	def compute_reward_reach_wall(self, state):
		...
		euclidean_penalty=((goal_x-x)**2+(goal_y-y)**2)*(-1)

		exploration_noise=np.random.uniform(size=1,low=-0.2, high=0.2)
		...
		# 	prev_x=self.prev_state[0]
		# 	prev_y=self.prev_state[1]
		# 	print('x_diff',abs(prev_x-x))
		# 	print('y_diff',abs(prev_y-y))
		# else:
		# 	print('no prev state') 

		# total_x=0
		# total_y=0
		# print('Rewards:',euclidean_penalty,exploration_noise)
		return (-1)*((goal_x-x)**2+(goal_y-y)**2)**2+wall_penalty+goal_state
